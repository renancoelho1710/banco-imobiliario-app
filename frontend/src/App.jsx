...
const socket = io("http://SEU_BACKEND:3001");
  ...
  if (!logado)
    return (
      <div className="login">
        ...
        <button onClick={() => { socket.emit("entrarSala", { sala, nome }); setLogado(true); }}>Entrar</button>
      </div>
    );
    ...
    <div className="app">
      ...
      {tela === "home" && (
        <>
          ...
          <h4>Jogadores</h4>
          {Object.entries(jogadores).map(([id, j]) => <div key={id}>{j.nome} — R$ {j.saldo}</div>)}
        </>
      )}
      ...
      {tela === "cobrar" && (
        <>
          ...
          {Object.entries(jogadores).filter(([id]) => id !== meuId).map(([id, j]) => (
            <button key={id} onClick={() => setQrData(JSON.stringify({ sala, de: id, para: meuId, valor: Number(valor), tipo: "aluguel" }))}>{j.nome}</button>
          ))}
      ...
      {tela === "extrato" && (
        <>
          {estado.extrato.map((e, i) => <div key={i}>{e.de} → {e.para} | R$ {e.valor}</div>)}
          <button onClick={() => setTela("home")}>Voltar</button>
        </>
      )}
    </div>
